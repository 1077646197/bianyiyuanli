...
结构体成员 → 成员类型 标识符 ( = 表达式 )? ;
成员类型 → int | void | float |char | struct 标识符
...
加法表达式 → 乘法表达式 ( (+|-) 乘法表达式 )*
乘法表达式 → 基本表达式 ( (*|/) 基本表达式 )*
+基本表达式 → 标识符( 数组访问后缀 | 函数调用后缀 )? | 数字 | ( 表达式 )
...
void parse_primary_expr();                            // 基本表达式 → 标识符 ( 数组访问后缀 | 函数调用后缀 )? | 数字 | ( 表达式 )
...
// 解析基本类型：basic_type → int | void | float | char
...
// 解析基本表达式：primary_expr → 标识符 ( 数组访问后缀 | 函数调用后缀 )? | 数字 | (表达式)
...
void parse_statement_list() {
    if (current_token &&
        (strstr(current_token, "(K 1)") || strstr(current_token, "(K 2)") || strstr(current_token, "(K 4)") || strstr(current_token, "(K 16)") ||  // 声明
